...
                console.assert(y1 == y2);
                return Math.abs(p1[1] - y1) + Math.abs(q1[1] - y1);
            }
        } else if (q1[1] == q2[1]) {
...
                return Math.abs(p1[0] - q1[0]) + Math.abs(p1[1] - q1[1]);
            }
        } else {
...
                return Math.abs(q1[0] - p1[0]) + Math.abs(p1[1] - q1[1]);
            }
        } else if (q1[1] == q2[1]) {
            if (p1[1] == q1[1]) {
...
                console.assert(x1 == x2);
                return Math.abs(p1[0] - x1) + Math.abs(q1[0] - x1);
            }
        } else {
            console.assert(0, "Wrong interval: ", i2);
        }
...
    let result = -1;
    for (let i = 0, sum1 = 0; i < input1.length; ++i) {
        for (let j = 0, sum2 = 0; j < input2.length; ++j) {
            let d = intersect(input1[i], input2[j]);
            if (d > 0) {
                let sum = d + sum1 + sum2;
                if (result == -1 || result > sum) {
//                     console.log("Intersect: " + sum, input1[i], input2[j])
                    result = sum;
                }
            }
            let q1 = input2[j][0];
            let q2 = input2[j][1];
            sum2 += Math.abs(q1[0] - q2[0]) + Math.abs(q1[1] - q2[1]);
        }
        let p1 = input1[i][0];
        let p2 = input1[i][1];
        sum1 += Math.abs(p1[0] - p2[0]) + Math.abs(p1[1] - p2[1]);
    }
    return result;
}
