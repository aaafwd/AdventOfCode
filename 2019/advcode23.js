...
function test(prog) {
  ...
  let programs = [];
  for (let i = 0; i < 50; ++i) {
    programs.push(new Program(Array.from(prog), inputs[i]));
  }
  let nat = undefined;
  let prev_nat = undefined;
  while (1) {
    let has_running = false;
    for (let i = 0; i < 50; ++i) {
      let program = programs[i];
      if (program.isDone()) continue;
      has_running = true;
      if (program.awaitsInput()) {
        program.provideInput(-1);
      } else {
        let target = program.consumeOutput();
        let x = program.consumeOutput();
        let y = program.consumeOutput();
        if (target == 255) {
          if (!nat) {
            console.log("Answer (part 1):", y);
          }
          if (programs.every(p => p.awaitsInput())) {
            if (prev_nat !== undefined && prev_nat[1] == y) {
              console.log("Answer (part 2):", y);
              return;
            }
            programs[0].provideInput(x);
            programs[0].provideInput(y);
            prev_nat = [x, y];
          }
          nat = [x, y];
        } else {
          inputs[target].push(x, y);
          if (programs[target].awaitsInput()) {
            programs[target].provideInput(inputs[target].shift());
          }
        }
      }
    }
